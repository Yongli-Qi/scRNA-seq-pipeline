setwd('/home/yq238/project/scRNA_seq/HTAN2/06coarse_annotation')
load('/home/yq238/project/scRNA_seq/HTAN2/05seurat_obj_integrated/seurat_integrated_fastmnn.RData')
...
write.table(rownames(ref@meta.data[ref@meta.data$cell_type_general=='Immune',]),'immune_cells_ref.txt',sep = '\t',quote = F, col.names = F, row.names = F)
write.table(rownames(ref@meta.data[ref@meta.data$cell_type_general=='Mesenchymal',]),'mesenchymal_cells_ref.txt',sep = '\t',quote = F, col.names = F, row.names = F)
write.table(rownames(ref@meta.data[ref@meta.data$cell_type_general=='Epithelial',]),'epithelial_cells_ref.txt',sep = '\t',quote = F, col.names = F, row.names = F)
